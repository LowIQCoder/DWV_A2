let scene, camera, renderer, globe;
        let points = [];
        const maxPointAge = 10000; // 10 seconds
        ...
        function updatePoints() {
            ...
            points = points.filter(point => {
                if(now - point.createdAt > maxPointAge) {
                    scene.remove(point);
                    return false;
                }
                
                // Fade out effect
                point.material.opacity = 1 - ((now - point.createdAt) / maxPointAge);
                return true;
            });
        }
        ...
        async function fetchData() {
            try {
                const response = await fetch('http://localhost:5001/packages');
